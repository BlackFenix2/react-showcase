...
import { playCross, playCircle, playGame, toggleMute } from "@/components/Views/TicTacToe/Sounds";
import Options from "@/components/Views/TicTacToe/Options";
import ScoreCard from "@/components/Views/TicTacToe/ScoreCard";
import Board from "@/components/Views/TicTacToe/Board";
import Debug from "@/components/Views/TicTacToe/Debug";
...
class TicTacToe extends React.Component<any, any> {
  ...
  boxClicked = async (e) => {
    ...
    if (this.state.gameEnded || this.state.gameLocked || this.state.numOfPlayers === 0 || e.target.textContent !== "") {
      return;
    }
  ...
  startTurn = async (loc) => {
    ...
    throw new Error(`Turn Error: Player ${this.state.turn} attempted to fill a non-empty slot: ${loc}`);
  };
  ...
  playSelf = async () => {
    if (
      ...
      !confirm(`Warning: The computer will play itself for ${this.state.warGamesCount} games`)
    ) {
      return;
    }
    this.reset();
    ...
    for (let i = 0; i < this.state.warGamesCount; i += 1) {
      ...
      this.setState(
        (prevState) => ({
          warGamesDelay: prevState.warGamesDelay - 100 >= 100 ? prevState.warGamesDelay - 100 : 50,
        }),
  ...
  aiTurn = async (players) => {
    ...
    if (!this.state.gameEnded) {
      ...
      const winningMoves = stats.filter((x) => x.winner === this.state.turn || x.winner === "draw");
      ...
      if (stats.length > 0) {
        try {
          ...
          const winningIndex = winningMoves.find((x) => x.totalMoves <= optimalWinningMoves).totalMoves;
          const numberPicked = winningMoves[winningIndex].boxOrder[this.state.totalMoves];
  ...
  oldCpuTurn = async (players) => {
    ...
    if (!this.state.gameEnded) {
      ...
      if (this.state.totalMoves === 0) {
        ...
        const diag = [0, 2, 6, 8];
        const acrossCheck =
          (this.state.board[1] === this.state.board[3]) === (this.state.board[5] === this.state.board[7]);
        const diagCheck =
          (this.state.board[0] === this.state.board[2]) === (this.state.board[6] === this.state.board[8]);
        if (!diagCheck) {
          this.startTurn(4);
        } else if (!acrossCheck) {
          if (this.state.board[5] !== this.state.board[3]) {
            ...
            do {
              random = innerPattern[Math.floor(Math.random() * innerPattern.length)];
            } while (this.state.board[random] !== "");
          } else {
        ...
        const pattern = this.state.board[4] === "O" ? [1, 3, 5, 7] : [0, 2, 6, 8];

        if (this.state.board[1] === this.state.board[3] && this.state.board[1] && this.state.board[3]) {
          // pattern 1: diagonal symbols
          this.startTurn(0);
        } else if (this.state.board[1] === this.state.board[5] && this.state.board[1] && this.state.board[5]) {
          this.startTurn(2);
        } else if (this.state.board[7] === this.state.board[3] && this.state.board[7] && this.state.board[3]) {
          this.startTurn(6);
        } else if (this.state.board[7] === this.state.board[5] && this.state.board[7] && this.state.board[5]) {
          this.startTurn(8);
        } else {
          // find unequal pattern
          if (this.state.board[1] !== this.state.board[7] || this.state.board[3] !== this.state.board[5]) {
            // check for hook patterns
            if (
              this.state.board[1] === this.state.board[6] &&
              this.state.board[2] === this.state.board[3] &&
              ((this.state.board[1] && this.state.board[6]) || (this.state.board[2] && this.state.board[3]))
            ) {
              this.startTurn(0);
              return;
            }
            if (
              ...
              ((this.state.board[0] && this.state.board[5]) || (this.state.board[1] && this.state.board[8]))
            ) {
              this.startTurn(2);
              return;
            }
            if (
              ...
              ((this.state.board[0] && this.state.board[7]) || (this.state.board[3] && this.state.board[8]))
            ) {
              this.startTurn(6);
              return;
            }
            if (
              ...
              ((this.state.board[2] && this.state.board[7]) || (this.state.board[5] && this.state.board[6]))
            ) {
              this.startTurn(8);
              return;
            }
      ...
      } else if (!(this.state.board[0] && this.state.board[2] && this.state.board[6] && this.state.board[8])) {
        // pick diagonal box
